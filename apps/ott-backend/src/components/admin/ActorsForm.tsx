...
import { addMovieActor, updateMovieActor, deleteMovieActor } from "@/actions/movies";
import { addTvActor, updateTvActor, deleteTvActor } from "@/actions/tvShows";
...
function empty(): ActorState {
  return {
    name: "", originalName: "", characterName: "", priority: "", image: "",
    bio: "", birthName: "", nickname: "", height: "", age: "",
    family: "", trademarks: "", trivia: "", quotes: "", otherWorks: "",
  };
}

function toPayload(s: ActorState) {
  const p = parseInt(s.priority, 10);
  const a = parseInt(s.age, 10);
  return {
    name: s.name.trim(),
    originalName: s.originalName.trim() || null,
    characterName: s.characterName.trim() || null,
    priority: isNaN(p) ? null : p,
...
interface EditPanelProps {
  state: ActorState;
  onChange: (update: Partial<ActorState>) => void;
  onSave: () => void;
  onCancel: () => void;
  pending: boolean;
}
    ...
    <div className="rounded-md border p-4 space-y-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <div className="space-y-1">
          ...
          <Input value={state.characterName} onChange={(e) => onChange({ characterName: e.target.value })} className="h-8" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">Priority</Label>
          <Input type="number" value={state.priority} onChange={(e) => onChange({ priority: e.target.value })} className="h-8" />
        </div>
        <div className="space-y-1">
      ...
      <div className="flex gap-2">
        <Button disabled={pending || !state.name.trim()} onClick={onSave}>
          {pending ? "Saving..." : "Save"}
        </Button>
...
export function ActorsForm({ type, contentId, actors }: Props) {
  const [pending, startTransition] = useTransition();
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editState, setEditState] = useState<ActorState>(empty());
  const [addState, setAddState] = useState<ActorState>(empty());
  const [showAdd, setShowAdd] = useState(false);

  function startEdit(a: Actor) {
    setEditingId(a.id);
    setEditState({
      name: a.name,
  ...
  function saveEdit(id: number) {
    ...
    startTransition(async () => {
      const result = type === "movie"
        ? await updateMovieActor(id, contentId, toPayload(editState))
        : await updateTvActor(id, contentId, toPayload(editState));
      if (result.success) {
  ...
  function handleDelete(id: number) {
    startTransition(async () => {
      const result = type === "movie"
        ? await deleteMovieActor(id, contentId)
        : await deleteTvActor(id, contentId);
      if (result.success) {
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      const result = type === "movie"
        ? await addMovieActor(contentId, toPayload(addState))
        : await addTvActor(contentId, toPayload(addState));
      if (result.success) {
  ...
  return (
    <div className="space-y-6">
      {actors.length > 0 ? (
              ...
              <tbody>
                {actors.map((a) => (
                  <tr key={a.id} className="border-b last:border-0">
                    <td className="px-3 py-2">
                      {a.image ? (
                        <div className="relative w-8 h-8 rounded-full overflow-hidden bg-muted">
                          <Image src={a.image} alt={a.name} fill className="object-cover" sizes="32px" />
                        </div>
                      ) : (
                    ...
                    <td className="px-3 py-2 text-muted-foreground">{a.characterName ?? "-"}</td>
                    <td className="px-3 py-2 text-muted-foreground">{a.priority ?? "-"}</td>
                    <td className="px-3 py-2">
                      <div className="flex gap-1">
                        <Button size="sm" variant="outline" onClick={() => { startEdit(a); setShowAdd(false); }}>Edit</Button>
                        <Button size="sm" variant="destructive" disabled={pending} onClick={() => handleDelete(a.id)}>Remove</Button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <p className="text-sm text-muted-foreground">No cast members.</p>
      )}
