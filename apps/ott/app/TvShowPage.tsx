...
import { filterTvShows } from "@/lib/actions-tv";
...
export function TvShowPage({ initialTvShows, genres, providers, initialFilters }: TvShowPageProps) {
  ...
  useEffect(() => {
    ...
    router.replace(queryString ? `/tv?${queryString}` : "/tv", { scroll: false });

    async function fetchFilteredTvShows() {
      setIsLoading(true);
      try {
        const filtered = await filterTvShows({
          ...filterState,
        ...
        <main className="flex-1 min-w-0">
          <div className="mb-4 flex items-center justify-between gap-2">
            <span className="text-sm text-zinc-400">
              {isLoading ? "Loading..." : `Showing ${tvShows.length} TV shows`}
            </span>
            <button
